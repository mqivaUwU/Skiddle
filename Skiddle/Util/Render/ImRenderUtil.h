...
public:
    ...
    inline static void drawInput(Vector2<float> pos, std::string* textStr, const UIColor& color, float textSize, float alpha, bool shadow = false, int index = 0)
    {
        if (!ImGui::GetCurrentContext())
            return;
    ...
    void drawSteve(Vector4<float> pos) {
