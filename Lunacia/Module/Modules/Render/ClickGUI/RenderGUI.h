...
public:
    ...
    void render(float animation, float inScale, int& scrollDirection)
    {
        ...
        */
        ImRenderUtil::fillRectangle(Vector4<float>(0, 0, ImRenderUtil::getScreenSize().x, ImRenderUtil::getScreenSize().y), UIColor(0, 0, 0), animation * 0.6f);
        ImRenderUtil::fillGradientOpaqueRectangle(Vector4<float>(0, 500, ImRenderUtil::getScreenSize().x, ImRenderUtil::getScreenSize().y), backColor, backColor2, animation * 0.5f, 0.f);
        ...
        if (!catPositions.empty())
        {
            for (size_t i = 0; i < categories.size(); i++)
            {
                ...
                Vector4<float> catWindow = Vector4<float>(catPositions[i].x, catPositions[i].y, catPositions[i].x + catWidth, catPositions[i].y + moduleY + catWindowHeight)
                    .scaleToPoint(Vector4<float>(ImRenderUtil::getScreenSize().x / 2, ImRenderUtil::getScreenSize().y / 2, ImRenderUtil::getScreenSize().x / 2, ImRenderUtil::getScreenSize().y / 2), inScale);
                ImRenderUtil::fillShadowRectangle(catWindow, UIColor(0, 0, 0), animation * 0.8f, 300, 0);
                ...
                for (const auto& mod : modsInCategory) {
                    ...
                    if (mod->getCategory() == categories[i]) {
                            ...
                            for (const auto& setting : mod->getSettings()) {
                                ...
                                case SettingType::BOOL: {
                                    ...
                                    if (rect.y > catRect.y + 0.5f) {
                                        ...
                                        ImRenderUtil::drawText(Vector2(rect.x + 5.f, cSetRectCentreY), &setName, *(bool*)setting->getValue() ? UIColor(255, 255, 255) : UIColor(180, 180, 180), textSize, animation, false);
                                ...
                                case SettingType::ENUM: {
                                    ...
                                    if (setting->enumSlide > 0.001)
                                    {
                                        for (int j = 0; j < numValues; j++)
                                        {
                                            ...
                                            if (rect2.y > catRect.y + 0.5f)
                                            {
                                                ...
                                                ImRenderUtil::drawText(Vector2<float>(rect2.x + 5.f, cSetRectCentreY), &enumValue, UIColor(255, 255, 255), textSize, animation, false);
                                    ...
                                    if (rect.y > catRect.y + 0.5f) {
                                        ...
                                        ImRenderUtil::drawText(Vector2<float>(rect.x + 5.f, cSetRectCentreY), &settingString, UIColor(255, 255, 255), textSize, animation, false);
                                ...
                                case SettingType::SLIDER: {
                                    ...
                                    if (rect.y > catRect.y + 0.5f)
                                    {
                                        ...
                                        ImRenderUtil::fillRectangle(Vector4<float>(rect.x, rect.y + 25, rect.x + setting->sliderEase, rect.w - 10), rgb, animation);                                        ImRenderUtil::fillRectangle(Vector4<float>(rect.x, rect.y + 30, rect.x + setting->sliderEase, rect.w), rgb, animation);
                                        // ImRenderUtil::fillShadowRectangle(Vector4<float>(rect.x, rect.y + 30, rect.x + setting->sliderEase, rect.w), UIColor(255, 255, 255), animation, 6.f, 0);                                        ImRenderUtil::fillRectangle(Vector4<float>(rect.x, rect.y + 30, rect.x + setting->sliderEase, rect.w), rgb, animation);

                                        ImRenderUtil::drawText(Vector2<float>(rect.x + 5.f, rect.y + 2.5f), &setName, UIColor(255, 255, 255), textSize, animation, false);
                                        ImRenderUtil::drawText(Vector2<float>(rect.x + 130.f, rect.y + 2.5f), &setVal, UIColor(200, 200, 200), textSize, animation, false);
                        ...
                        if (modRect.y > catRect.y + 0.5f)
                        {
                            ...
                            // Draw the string
                            ImRenderUtil::drawText(modPosLerped, &modName, UIColor(mod->isEnabled() ? UIColor(200, 200, 200) : UIColor(120, 120, 120)), textSize, animation, true);
                            // ImRenderUtil::drawText(modPosLerped, &modName, UIColor(mod->isEnabled() ? UIColor(120, 120, 120) : UIColor(180, 180, 180)).lerp(UIColor(100, 100, 100), mod->cAnim), textSize, animation, false);
                            // ImRenderUtil::drawText(modPosLerped, &modName, UIColor(mod->isEnabled() ? UIColor(120, 120, 120) : UIColor(180, 180, 180)).lerp(UIColor(100, 100, 100), mod->cAnim), textSize, animation, true);
                            ...
                            if (ImRenderUtil::isMouseOver(modRect) && catPositions[i].isExtended && isEnabled)
                            {
                                ...
                                if (Util::leftDown)
                                {
                                    mod->toggle();
                                    Util::leftDown = false;
                                    particleMgr.addParticles(5, GuiInfo::MousePos.x, GuiInfo::MousePos.y, 80, 2.f, 1);
                                }
                ...
                // Draw the string
                ImRenderUtil::drawText(Vector2(cRectCentreX, cRectCentreY), &catName, UIColor(255, 255, 255), textSize, animation, false);
                ImRenderUtil::drawText(Vector2(cRectCentreX -= 0.0001, cRectCentreY += 0.00005), &catName, UIColor(255, 255, 255), textSize, animation, false);
