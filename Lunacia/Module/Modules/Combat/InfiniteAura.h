...
{
public:
    InfiniteAura(int keybind = Keyboard::NONE, bool enabled = false) :
        Module("InfiniteAura", "Combat", "Serversidedly teleport to people and attack them", keybind, enabled)
    {
        registerEnumSetting("Mode", "The Mode", { "PocketMine", "MovePlayer" }, &mode);
        registerEnumSetting("Target", "How many entities will be targeted", { "Single", "Multi" }, &targetMode);
        registerFloatSetting("Range", "How far it will search for entities", &range, 0, 200);
        registerFloatSetting("Steps", "How many steps it will take to get to an entity", &teleportIncrement, 0.1f, 20);
        registerBoolSetting("Raycast", "Only attack entities you can see", &raytrace);
        registerFloatSetting("APS", "How fast you will attack", &delayMS, 0, 20);
