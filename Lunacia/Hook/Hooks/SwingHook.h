...
float getInterpolatedBodyYaw(Player* _this, float a1) {
    if (_this == Game::GetLocalPlayer() && getModuleByName("aura")->isEnabled() && !Game::TargetLists::auraList.empty()/* || _this == Game::GetLocalPlayer() && getModuleByName("scaffold")->isEnabled()*/) {
...
float* yRot(Player* _this, float* a2, float a3) {
    ...
    if (_this == Game::GetLocalPlayer() && getModuleByName(xorstr_("aura"))->isEnabled() && !Game::TargetLists::auraList.empty() || _this == Game::GetLocalPlayer() && getModuleByName(xorstr_("scaffold"))->isEnabled() || _this == Game::GetLocalPlayer() && getModuleByName(xorstr_("regen"))->isEnabled()) {
